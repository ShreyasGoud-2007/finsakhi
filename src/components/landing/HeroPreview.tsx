...
export function HeroPreview() {
  const { t } = useStore();
  const slices = [
    { label: t("category.food"), pct: 41, color: "#C6462E" },
    { label: t("category.education"), pct: 27, color: "#2F6DB0" },
    { label: t("category.transport"), pct: 14, color: "#1E8C79" },
    { label: t("category.household"), pct: 12, color: "#C98A1E" },
    { label: t("category.healthcare"), pct: 6, color: "#9B4D8E" },
  ];
  ...
  return (
    <div className="relative mx-auto w-full max-w-[400px]" aria-hidden="true">
      <div className="rounded-3xl border border-ink-300/25 bg-white p-5 shadow-lift">
        <p className="font-display text-lg font-bold">{t("landing.previewGreeting")}</p>
        <p className="text-sm text-ink-500">{t("label.subtitle")}</p>

        <div className="mt-4 grid grid-cols-3 gap-2.5">
          {[
            { icon: ArrowUpCircle, label: t("landing.previewIn"), value: "₹15,000", cls: "bg-income-soft text-income" },
            { icon: ArrowDownCircle, label: t("landing.previewOut"), value: "₹8,500", cls: "bg-expense-soft text-expense" },
            { icon: Wallet, label: t("landing.previewLeft"), value: "₹6,500", cls: "bg-brand-50 text-brand-700" },
          ].map((c) => (
        ...
        <div className="mt-4 rounded-2xl border border-ink-300/20 p-4">
          <p className="text-sm font-semibold text-ink-700">{t("label.spending")}</p>
        ...
        <div className="mt-4 rounded-2xl border-2 border-brand-200 bg-brand-50 p-4">
          <p className="flex items-center gap-2 text-sm font-bold text-brand-700">
            <Sparkles size={16} /> {t("landing.previewInsight")}
          </p>
          <p className="mt-1.5 text-sm leading-relaxed text-ink-900">
            {t("landing.previewInsightText")}
          </p>
        </div>
      </div>
      ...
      <div className="decorative absolute -bottom-5 -left-4 hidden sm:block rounded-2xl
        ...
        <p className="text-xs font-semibold text-ink-500">{t("landing.previewGoal")}</p>
