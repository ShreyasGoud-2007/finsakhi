/**
 * StaggeredMenu — React Bits (JS-CSS variant)
 * Source: https://github.com/DavidHDev/react-bits
 * License: MIT + Commons Clause
 *
 * Copied verbatim from upstream; CSS import points to FinSakhi-themed override file.
 * The logoUrl prop is not used in the FinSakhi integration (logo hidden via CSS).
 */
  ...
  return (
    ...
    >
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {(() => {
          const raw = colors?.length ? colors.slice(0, 4) : ['#EAF5F1', '#0F766E'];
          let arr = [...raw];
      ...
      <header className="staggered-menu-header" aria-label="Mobile navigation">
        {/* logo area intentionally empty — FinSakhi brand is in the main navbar */}
            ...
            {items?.length ? (
              items.map((it, idx) => (
                <li className="sm-panel-itemWrap" key={it.label + idx}>
                  {it.onClick ? (
                    /* button-style item (e.g. Get Started) */
                    <button
                      type="button"
